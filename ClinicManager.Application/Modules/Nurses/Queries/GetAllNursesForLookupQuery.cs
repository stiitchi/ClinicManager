...
{
    public class GetAllNursesForLookupQuery : IRequest<Result<List<LookupDTO>>>
    {
    }

    public class GetAllNursesForLookupQueryHandler : IRequestHandler<GetAllNursesForLookupQuery, Result<List<LookupDTO>>>
    {
        private readonly IApplicationDbContext _context;

        public GetAllNursesForLookupQueryHandler(IApplicationDbContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));
        }

        public async Task<Result<List<LookupDTO>>> Handle(GetAllNursesForLookupQuery request, CancellationToken cancellationToken)
        {
            try
            {
