...
{
    public class GetAllRecentAdmissionsTableQuery : IRequest<PaginatedResult<PatientDTO>>
    {
        ...
        public GetAllRecentAdmissionsTableQuery(int pageNumber, int pageSize, string searchString, string orderBy)
        {
    ...
    public class GetAllRecentAdmissionsTableQueryHandler : IRequestHandler<GetAllRecentAdmissionsTableQuery, PaginatedResult<PatientDTO>>
    {
        private readonly IApplicationDbContext _context;

        public GetAllRecentAdmissionsTableQueryHandler(IApplicationDbContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));
        }

        public async Task<PaginatedResult<PatientDTO>> Handle(GetAllRecentAdmissionsTableQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                DateTime weekInterval = DateTime.Now.AddDays(-7);
                ...
                if (request.OrderBy?.Any() != true)
                {
                   ...
                   .AsNoTracking()
                   .IgnoreQueryFilters()
                   .Where(x => x.AdmissionDate > weekInterval)
                   .Select(expression)
